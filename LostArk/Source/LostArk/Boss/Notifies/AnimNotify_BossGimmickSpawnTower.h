...
public:
	...
	/**
	 * 이번 라운드에 파괴할 슬라이스(0 ~ SliceCount-1). 슬라이스 인덱스 직접 지정.
	 * 이 한 값이 이번 라운드의 바라보기(FaceSlice)/파괴(DestroySlice)/타워 제외를 모두 결정한다.
	 */
	UPROPERTY(EditAnywhere, Category = "Gimmick", meta = (ClampMin = "0"))
	int32 TargetSlice = 0;
